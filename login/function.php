...
class user
 {
    ...
    {
        $mysqli = mysqli_connect("localhost","root","","car_rental");
        if(mysqli_connect_errno()){
            die("Error 01: cannot connect to Database <a href='#'>Report this error</a>". mysqli_connect_error());
        }
        $encrypt = md5($password);
        $query = "SELECT * FROM customer WHERE email = '$username' AND password = '$encrypt'";
        ...
        if ( $count == 1 ){
            ...
            $_SESSION ["cusEmail"] = $username;
            $_SESSION ["cusPw"] = $encrypt;
...
class register{
    ...
    public function registerUser($name, $address)
    {
        ...
        if($userCount < 1){
                $qry = "INSERT INTO customer VALUES 
                    (NULL,'$name',  '$address',   '$this->phone', '$this->email', '$this->password')";
